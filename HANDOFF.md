...
> ### ✅ 第十五轮（2026-09-26）：面板与"洞"错位（两套视口坐标系）+ 外观弹窗被切
>
> 用户（Android Chromium PWA）：「去除遮罩的地方和弹窗真正大小不匹配」「外观的弹窗也保证不被遮挡居中屏幕」。
> 两件事两个根因：
>
> ① **面板与洞用了两套坐标系**：定位层是 `absolute`（包含块＝**布局**视口），遮罩是 `fixed`（基准＝**可见**视口）。
>    手机浏览器里两者不重合（地址栏/系统栏高度差）⇒ 整体错位。本地无头复现不出来（那里两个视口恒等）。
>    修法：portal 的根套一层 `.gs-viewport{position:fixed;inset:0}`，两者共用基准；遮罩改 `absolute`。
>    材质不受影响（保留率仍 0.02）；该层是 fixed ⇒ 自成 stacking context，z 镜像 `--gs-z`（桌面 70 / 手机 130），
>    手机 dock 与菜单的相对顺序不变（实测 dock 区域仍被压暗 −7.0、对比度 0.72，点它被遮罩接住）。
> ② **外观弹窗被切**：限高只减 `--app-height`，而它在 standalone 下是 **100lvh**（可能大于可见高度）
>    ⇒ 面板比屏幕还高。实测：设 1000px 时上下各越界 17px、1200px 时 78px。修法：改成
>    `calc(min(100svh, var(--app-height, 100dvh)) - N)`，两档都改。修后 1000/1200px 下都是 0/0。
>
> 验收：洞 vs 面板 **23.7,47.7 354.0×778.3** vs **24,46 354×782**（差≤2px＝边缘 AA）；
> 两引擎 `mobile-dialog-verify` 全绿、桌面档全绿、dock 层序不变、回归全绿。
>
> **已推送**（见文末推送记录）。
>
